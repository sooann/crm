...
class GeneratorConfig implements GeneratorConfigInterface
{
    ...
    public function setBuildProperties($props)
    {
        ...
        foreach ($props as $key => $propValue) {
            if (strpos($key, $mainClass) === 0) {
                $newKey = substr($key, strlen($mainClass));
                $j = strpos($newKey, '.');
                while ($j !== false) {
	...
	/** 
	* Get configured behaviour class parameters
	*/
	
	public function getConfiguredBehaviourParameter($name, $param) {
		$propname = 'behavior'. ucfirst(strtolower($name)) . 'Parameter' . ucfirst(strtolower($param)) ;
        try {
            $ret = $this->getClassname($propname);
        } catch (BuildException $e) {
            // class path not configured
	...
	public function setMainClass($name)
	{
		$this->mainClass = $name;
	}
