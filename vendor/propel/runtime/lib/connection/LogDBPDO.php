...
class DebugPDO extends PropelPDO
{
	...
	public function log($msg, $level = null, $methodName = null, array $debugSnapshot = null) {
		...
		//log all execute statements
				
		$sqltablename = $this->getConfiguration()->getParameter("logdbpdo.logtable", "sec_sqlaudit");
		
		if ($msg!=null) {
			$modifiedColumns[':p' . $index++]  = '`STRSQLSTATEMENT`';
		}
		
        $startTime = PropelDateTime::newInstance($debugSnapshot['microtime'];, null, 'DateTime');
        if ($startTime !== null) {
			$startTime = $startTime ? $startTime->format('Y-m-d H:i:s') : null;
			$modifiedColumns[':p' . $index++]  = '`DTSTARTTIME`';
		}
		
		$now = $this->getDebugSnapshot();
		$endTime = PropelDateTime::newInstance($now['microtime'] ;, null, 'DateTime');
        if ($endTime !== null) {
			$endTime = $endTime ? $endTime->format('Y-m-d H:i:s') : null;
			$modifiedColumns[':p' . $index++]  = '`DTENDTIME`';
		}
		
		
		
        $sql = sprintf(
            'INSERT INTO `'.$sqltablename.'` (%s) VALUES (%s)',
            implode(', ', $modifiedColumns),
            implode(', ', array_keys($modifiedColumns))
        );
		
		
		return parent::log($msg, $level = null, $methodName = null, array $debugSnapshot = null);
	}
