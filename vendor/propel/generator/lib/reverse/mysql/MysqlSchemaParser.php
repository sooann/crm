...
class MysqlSchemaParser extends BaseSchemaParser
{
    ...
    protected function addColumns(Table $table)
    {
		...
		$require_timestamp_behavior_CreatedDate = false;
		$require_timestamp_behavior_ModifiedDate = false;
		
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $column = $this->getColumnFromRow($row, $table);
            $table->addColumn($column);

			//check if table requires timestamp behaviour
			if (strtolower($row['Field'])==strtolower("dtCreatedDate")) {
				$require_timestamp_behavior_CreatedDate=true;
			} 
			
			if (strtolower($row['Field'])==strtolower("dtModifiedDate")) {
				$require_timestamp_behavior_ModifiedDate=true;
			}
						
        }
		
		//if requre timestamp behaviour add behaviour
		if ($require_timestamp_behavior_CreatedDate || $require_timestamp_behavior_ModifiedDate) {
			//add behavior
			$behavior = new Behavior();
			
			//set parameters
			$args = array();
			if ($require_timestamp_behavior_CreatedDate) {
				$args['create_column'] = 'dtCreatedDate';
				$args['create_by'] = 'intCreatedBy';
			}
			
			if ($require_timestamp_behavior_ModifiedDate) {
				$args['update_column'] = 'dtModifiedDate';
				$args['update_by'] = 'intModifiedBy';
				$args['disable_updated_at'] = 'false';
			} else {
				$args['disable_updated_at'] = 'true';
			}
			
			$behavior->setParameters($args);
