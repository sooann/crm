...
class PhpNameGenerator implements NameGenerator
{
    ...
    public function generateName($inputs)
    {
        ...
        switch ($method) {
            ...
            case self::CONV_METHOD_NOCHANGE:
                ...
                break;
			case self::CONV_METHOD_DATABASE:
	            $phpName = $this->databaseMethod($schemaName);
	            break;				
    ...
    /**
     * Tool to reformat name like "int", "str", "dt", "dbl", "lng", "txt", "bln"
     *
     * T
     ...
     */
	 ...
	 protected function databaseMethod($schemaName) {
	 	
		 //find for "int", "str", "dbl", "lng", "txt", "bln"
		 $arrTest = array("int", "str", "dbl", "lng", "txt", "bln");
		 $strTest  = substr($schemaName,0,3);
		 if ( array_search($strTest,$arrTest) !== false) {
		 	
			 //exclude "string" and "integer"
			 if (strtolower(substr($schemaName,0,6))=="string" || strtolower(substr($schemaName,0,7))=="integer") {
				 $name = $schemaName;
			 } else {
				 $name = substr($schemaName,3,strlen($schemaName)-2);
			 }
			 ...
			 //find for "dt"
			 $arrTest = array("dt");
			 $strTest  = substr($schemaName,0,2);
			 if ( array_search($strTest,$arrTest) !== false) {
			 	$name = substr($schemaName,2,strlen($schemaName)-1);
			 } else {
				 $name = $schemaName;
			 }
		 }
