...
/* 
 * jsTree one-level dynamic CRRM plugin
 * Handles creating/renaming/removing/moving nodes by user interaction.
 */

(function ($) {
	$.jstree.plugin("onelevelcrrm", { 
		__init : function () {
			this.get_container()
				.bind("move_node.jstree", $.proxy(function (e, data) {
					if(this._get_settings().onelevelcrrm.move.open_onmove) {
						var t = this;
						data.rslt.np.parentsUntil(".jstree").andSelf().filter(".jstree-closed").each(function () {
							t.open_node(this, false, true);
						});
		...
		_fn : {
			_show_option : function (obj,callback) {
				obj = this._get_node(obj);
				...
				_show_parameter_array : function (obj, callback) {
					obj = this._get_node(obj);
					var text = this.get_text(obj);
					if (text==this._get_string("new_node")) { text='"key"=>"value"'; this.set_text(obj,text); }
					var key = text.split("=>")[0];
					...
					t = this.create_node(obj, position, js, function (t) {
						...
						if(!skip_rename) { 
							if (this.get_text(obj).indexOf("{array}")!==-1) {
								...
								this._show_parameter_array(t, function (obj, new_name, old_name) { 
									_this.__callback({ "obj" : obj, "name" : new_name, "parent" : p, "position" : pos });
								});
							} else {
				...
				check_move : function () {
					if(!this.__call_old()) { return false; }
					var s = this._get_settings().onelevelcrrm.move;
					if(!s.check_move.call(this, this._get_move())) { return false; }
					return true;
				},
				move_node : function (obj, ref, position, is_copy, is_prepared, skip_check) {
				...
				cut : function (obj) {
					obj = this._get_node(obj, true);
					if(!obj || !obj.length) { return false; }
					this.data.onelevelcrrm.cp_nodes = false;
					this.data.onelevelcrrm.ct_nodes = obj;
					this.__callback({ "obj" : obj });
				},
				copy : function (obj) {
					obj = this._get_node(obj, true);
					if(!obj || !obj.length) { return false; }
					this.data.onelevelcrrm.ct_nodes = false;
					this.data.onelevelcrrm.cp_nodes = obj;
					this.__callback({ "obj" : obj });
				},
				paste : function (obj) { 
					obj = this._get_node(obj);
					if(!obj || !obj.length) { return false; }
					var nodes = this.data.onelevelcrrm.ct_nodes ? this.data.onelevelcrrm.ct_nodes : this.data.onelevelcrrm.cp_nodes;
					if(!this.data.onelevelcrrm.ct_nodes && !this.data.onelevelcrrm.cp_nodes) { return false; }
					if(this.data.onelevelcrrm.ct_nodes) { this.move_node(this.data.onelevelcrrm.ct_nodes, obj); this.data.crrm.ct_nodes = false; }
					if(this.data.onelevelcrrm.cp_nodes) { this.move_node(this.data.onelevelcrrm.cp_nodes, obj, false, true); }
					this.__callback({ "obj" : obj, "nodes" : nodes });
				}
			} 
		});
...
/* 
 * jsTree Clonefish CRRM plugin
 * Handles creating/renaming/removing/moving nodes by user interaction.
 */
(function ($) {
	$.jstree.plugin("clonefishcrrm", { 
		__init : function () {
			this.get_container()
				.bind("move_node.jstree", $.proxy(function (e, data) {
					if(this._get_settings().clonefishcrrm.move.open_onmove) {
						var t = this;
						data.rslt.np.parentsUntil(".jstree").andSelf().filter(".jstree-closed").each(function () {
							t.open_node(this, false, true);
						});
		...
		_fn : {
			_show_clonefish_option : function (obj,callback) {
				obj = this._get_node(obj);
				var text = this.get_text(obj);
				if (text==this._get_string("new_node")) { text='"key"=>"value"'; this.set_text(obj,text); }
				var key = text.split("=>")[0];
				...
				var rtl = this._get_settings().core.rtl,
					...
					h7 = obj.css("position","relative").append(
						$("<select />", {
							"class" : "jstree-rename-selectvalue",
							"css" : {
								"padding" : "0",
								"border" : "1px solid silver",
								"position" : "absolute",
			...
			check_move : function () {
				if(!this.__call_old()) { return false; }
				var s = this._get_settings().clonefishcrrm.move;
				if(!s.check_move.call(this, this._get_move())) { return false; }
				return true;
			},
			move_node : function (obj, ref, position, is_copy, is_prepared, skip_check) {
			...
			cut : function (obj) {
				obj = this._get_node(obj, true);
				if(!obj || !obj.length) { return false; }
				this.data.clonefishcrrm.cp_nodes = false;
				this.data.clonefishcrrm.ct_nodes = obj;
				this.__callback({ "obj" : obj });
			},
			copy : function (obj) {
				obj = this._get_node(obj, true);
				if(!obj || !obj.length) { return false; }
				this.data.clonefishcrrm.ct_nodes = false;
				this.data.clonefishcrrm.cp_nodes = obj;
				this.__callback({ "obj" : obj });
			},
			paste : function (obj) { 
				obj = this._get_node(obj);
				if(!obj || !obj.length) { return false; }
				var nodes = this.data.clonefishcrrm.ct_nodes ? this.data.clonefishcrrm.ct_nodes : this.data.clonefishcrrm.cp_nodes;
				if(!this.data.clonefishcrrm.ct_nodes && !this.data.clonefishcrrm.cp_nodes) { return false; }
				if(this.data.clonefishcrrm.ct_nodes) { this.move_node(this.data.clonefishcrrm.ct_nodes, obj); this.data.clonefishcrrm.ct_nodes = false; }
				if(this.data.clonefishcrrm.cp_nodes) { this.move_node(this.data.clonefishcrrm.cp_nodes, obj, false, true); }
				this.__callback({ "obj" : obj, "nodes" : nodes });
			}
		}
	});
	// include the crr plugin by default
	// $.jstree.defaults.plugins.push("clonefishcrrm");
})(jQuery);
//*/
	...
	$.jstree.plugin("contextmenu", {
		...
		_fn : {
			show_contextmenu : function (obj, x, y) {
				...
				} else if (this.data.clonefishcrrm!=undefined) {
                   var checktype=false;
                   var tobj = this._get_parent(obj);
