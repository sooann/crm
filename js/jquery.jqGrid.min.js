...
c.p.id+"_"+a+'"/>';return'<td role="gridcell" '+l(b,d,"",null,a,true)+">"+e+"</td>"},t=function(a,c,b,d){b=(parseInt(b,10)-1)*parseInt(d,10)+1+c;return'<td role="gridcell" class="ui-state-default jqgrid-rownum" '+l(a,c,b,null,c,true)+">"+b+"</td>"},v=function(a){var b,d=[],e=0,f;for(f=0;f<c.p.colModel.length;f++){b=c.p.colModel[f];if(b.name!=="cb"&&b.name!=="subgrid"&&b.name!=="rn"){d[e]=a=="local"?b.name:a=="xml"||a==="xmlstring"?b.xmlmap||b.name:b.jsonmap||b.name;e++}}return d},E=function(b){var d=
c.p.remapColumns;if(!d||!d.length)d=a.map(c.p.colModel,function(a,c){return c});b&&(d=a.map(d,function(a){return a<b?null:a-b}));return d},y=function(c,b){var d;if(this.p.deepempty)a(this.rows).slice(1).remove();else{d=this.rows.length>0?this.rows[0]:null;a(this.firstChild).empty().append(d)}if(c&&this.p.scroll){a(this.grid.bDiv.firstChild).css({height:"auto"});a(this.grid.bDiv.firstChild.firstChild).css({height:0,display:"none"});if(this.grid.bDiv.scrollTop!==0)this.grid.bDiv.scrollTop=0}if(b===
true&&this.p.treeGrid){this.p.data=[];this.p._index={}}},x=function(){var b=c.p.data.length,d,e,f;d=c.p.rownumbers===true?1:0;e=c.p.multiselect===true?1:0;f=c.p.subGrid===true?1:0;d=c.p.keyIndex===false||c.p.loadonce===true?c.p.localReader.id:c.p.colModel[c.p.keyIndex+e+f+d].name;for(e=0;e<b;e++){f=a.jgrid.getAccessor(c.p.data[e],d);c.p._index[f]=e}},B=function(b,d,e,f,g){var h="-1",i="",j,d=d?"display:none;":"",e="ui-widget-content jqgrow ui-row-"+c.p.direction+e,f=a.isFunction(c.p.rowattr)?c.p.rowattr.call(c,
f,g):{};if(!a.isEmptyObject(f)){if(f.hasOwnProperty("id")){b=f.id;delete f.id}if(f.hasOwnProperty("tabindex")){h=f.tabindex;delete f.tabindex}if(f.hasOwnProperty("style")){d=d+f.style;delete f.style}if(f.hasOwnProperty("class")){e=e+(" "+f["class"]);delete f["class"]}try{delete f.role}catch(l){}for(j in f)f.hasOwnProperty(j)&&(i=i+(" "+j+"="+f[j]))}return'<tr role="row" id="'+b+'" tabindex="'+h+'" class="'+e+'"'+(d===""?"":' style="'+d+'"')+i+">"},A=function(b,d,e,f,g){var h=new Date,i=c.p.datatype!=
"local"&&c.p.loadonce||c.p.datatype=="xmlstring",j=c.p.xmlReader,l=c.p.datatype=="local"?"local":"xml";if(i){c.p.data=[];c.p._index={};c.p.localReader.id="_id_"}c.p.reccount=0;if(a.isXMLDoc(b)){if(c.p.treeANode===-1&&!c.p.scroll){y.call(c,false,true);e=1}else e=e>1?e:1;var k,n,m=0,o,p=c.p.multiselect===true?1:0,q=c.p.subGrid===true?1:0,x=c.p.rownumbers===true?1:0,C,u=[],G,z={},A,D,w=[],F=c.p.altRows===true?" "+c.p.altclass:"",Q;j.repeatitems||(u=v(l));C=c.p.keyIndex===false?a.isFunction(j.id)?j.id.call(c,
...
j.records)||0;a.isFunction(j.userdata)?c.p.userData=j.userdata.call(c,b)||{}:a.jgrid.getXmlData(b,j.userdata,true).each(function(){c.p.userData[this.getAttribute("name")]=a(this).text()});b=a.jgrid.getXmlData(b,j.root,true);(b=a.jgrid.getXmlData(b,j.row,true))||(b=[]);var M=b.length,K=0,O=[],H=parseInt(c.p.rowNum,10);if(M>0&&c.p.page<=0)c.p.page=1;if(b&&M){var I=c.p.scroll?a.jgrid.randId():1;g&&(H=H*(g+1));for(var g=a.isFunction(c.p.afterInsertRow),ba=c.p.grouping&&c.p.groupingView.groupCollapse===
true;K<M;){A=b[K];D=l(A,I+K);D=c.p.idPrefix+D;k=e===0?0:e+1;Q=(k+K)%2==1?F:"";var V=w.length;w.push("");x&&w.push(t(0,K,c.p.page,c.p.rowNum));p&&w.push(r(D,x,K));q&&w.push(a(c).jqGrid("addSubGridCell",p+x,K+e));if(j.repeatitems){G||(G=E(p+q+x));var N=a.jgrid.getXmlData(A,j.cell,true);a.each(G,function(a){var b=N[this];if(!b)return false;o=b.textContent||b.text;z[c.p.colModel[a+p+q+x].name]=o;w.push(s(D,o,a+p+q+x,K+e,A))})}else for(k=0;k<u.length;k++){o=a.jgrid.getXmlData(A,u[k]);z[c.p.colModel[k+
p+q+x].name]=o;w.push(s(D,o,k+p+q+x,K+e,A))}w[V]=B(D,ba,Q,z,A);w.push("</tr>");if(c.p.grouping){O=a(c).jqGrid("groupingPrepare",w,O,z,K);w=[]}if(i||c.p.treeGrid===true){z._id_=D;c.p.data.push(z);c.p._index[D]=c.p.data.length-1}if(c.p.gridview===false){a("tbody:first",d).append(w.join(""));a(c).triggerHandler("jqGridAfterInsertRow",[D,z,A]);g&&c.p.afterInsertRow.call(c,D,z,A);w=[]}z={};m++;K++;if(m==H)break}}if(c.p.gridview===true){n=c.p.treeANode>-1?c.p.treeANode:0;if(c.p.grouping){a(c).jqGrid("groupingRender",
O,c.p.colModel.length);O=null}else c.p.treeGrid===true&&n>0?a(c.rows[n]).after(w.join("")):a("tbody:first",d).append(w.join(""))}if(c.p.subGrid===true)try{a(c).jqGrid("addSubGrid",p+x)}catch(J){}c.p.totaltime=new Date-h;if(m>0&&c.p.records===0)c.p.records=M;w=null;if(c.p.treeGrid===true)try{a(c).jqGrid("setTreeNode",n+1,m+n+1)}catch(P){}if(!c.p.treeGrid&&!c.p.scroll)c.grid.bDiv.scrollTop=0;c.p.reccount=m;c.p.treeANode=-1;c.p.userDataOnFooter&&a(c).jqGrid("footerData","set",c.p.userData,true);if(i){c.p.records=
M;c.p.lastpage=Math.ceil(M/H)}f||c.updatepager(false,true);if(i)for(;m<M;){A=b[m];D=l(A,m+I);D=c.p.idPrefix+D;if(j.repeatitems){G||(G=E(p+q+x));var L=a.jgrid.getXmlData(A,j.cell,true);a.each(G,function(a){var b=L[this];if(!b)return false;o=b.textContent||b.text;z[c.p.colModel[a+p+q+x].name]=o})}else for(k=0;k<u.length;k++){o=a.jgrid.getXmlData(A,u[k]);z[c.p.colModel[k+p+q+x].name]=o}z._id_=D;c.p.data.push(z);c.p._index[D]=c.p.data.length-1;z={};m++}}},F=function(b,d,e,f,g){d=new Date;if(b){if(c.p.treeANode===
...
F*(g+1));for(var Q=a.isFunction(c.p.afterInsertRow),O=[],H=c.p.grouping&&c.p.groupingView.groupCollapse===true;b<C;){g=u[b];w=a.jgrid.getAccessor(g,i);if(w===void 0){w=M+b;if(m.length===0&&h.cell){k=a.jgrid.getAccessor(g,h.cell);w=k!==void 0?k[i]||w:w}}w=c.p.idPrefix+w;k=e===1?0:e;K=(k+b)%2==1?A:"";var I=D.length;D.push("");x&&D.push(t(0,b,c.p.page,c.p.rowNum));p&&D.push(r(w,x,b));q&&D.push(a(c).jqGrid("addSubGridCell",p+x,b+e));if(h.repeatitems){h.cell&&(g=a.jgrid.getAccessor(g,h.cell));o||(o=E(p+
q+x))}for(n=0;n<o.length;n++){k=a.jgrid.getAccessor(g,o[n]);D.push(s(w,k,n+p+q+x,b+e,g));G[c.p.colModel[n+p+q+x].name]=k}D[I]=B(w,H,K,G,g);D.push("</tr>");if(c.p.grouping){O=a(c).jqGrid("groupingPrepare",D,O,G,b);D=[]}if(j||c.p.treeGrid===true){G._id_=w;c.p.data.push(G);c.p._index[w]=c.p.data.length-1}if(c.p.gridview===false){a("#"+a.jgrid.jqID(c.p.id)+" tbody:first").append(D.join(""));a(c).triggerHandler("jqGridAfterInsertRow",[w,G,g]);Q&&c.p.afterInsertRow.call(c,w,G,g);D=[]}G={};l++;b++;if(l==
F)break}if(c.p.gridview===true){z=c.p.treeANode>-1?c.p.treeANode:0;c.p.grouping?a(c).jqGrid("groupingRender",O,c.p.colModel.length):c.p.treeGrid===true&&z>0?a(c.rows[z]).after(D.join("")):a("#"+a.jgrid.jqID(c.p.id)+" tbody:first").append(D.join(""))}if(c.p.subGrid===true)try{a(c).jqGrid("addSubGrid",p+x)}catch(ba){}c.p.totaltime=new Date-d;if(l>0&&c.p.records===0)c.p.records=C;if(c.p.treeGrid===true)try{a(c).jqGrid("setTreeNode",z+1,l+z+1)}catch(V){}if(!c.p.treeGrid&&!c.p.scroll)c.grid.bDiv.scrollTop=
0;c.p.reccount=l;c.p.treeANode=-1;c.p.userDataOnFooter&&a(c).jqGrid("footerData","set",c.p.userData,true);if(j){c.p.records=C;c.p.lastpage=Math.ceil(C/F)}f||c.updatepager(false,true);if(j)for(;l<C&&u[l];){g=u[l];w=a.jgrid.getAccessor(g,i);if(w===void 0){w=M+l;m.length===0&&h.cell&&(w=a.jgrid.getAccessor(g,h.cell)[i]||w)}if(g){w=c.p.idPrefix+w;if(h.repeatitems){h.cell&&(g=a.jgrid.getAccessor(g,h.cell));o||(o=E(p+q+x))}for(n=0;n<o.length;n++){k=a.jgrid.getAccessor(g,o[n]);G[c.p.colModel[n+p+q+x].name]=
k}G._id_=w;c.p.data.push(G);c.p._index[w]=c.p.data.length-1;G={}}l++}}},u=function(){function b(d){var e=0,g,h,i,j,l;if(d.groups!==void 0){(h=d.groups.length&&d.groupOp.toString().toUpperCase()==="OR")&&p.orBegin();for(g=0;g<d.groups.length;g++){e>0&&h&&p.or();try{b(d.groups[g])}catch(k){alert(k)}e++}h&&p.orEnd()}if(d.rules!==void 0){if(e>0){h=p.select();p=a.jgrid.from(h);c.p.ignoreCase&&(p=p.ignoreCase())}try{(i=d.rules.length&&d.groupOp.toString().toUpperCase()==="OR")&&p.orBegin();for(g=0;g<d.rules.length;g++){l=
d.rules[g];j=d.groupOp.toString().toUpperCase();if(o[l.op]&&l.field){e>0&&j&&j==="OR"&&(p=p.or());p=o[l.op](p,j)(l.field,l.data,f[l.field])}e++}i&&p.orEnd()}catch(n){alert(n)}}}var d,e=false,f={},g=[],h=[],i,j,l;if(a.isArray(c.p.data)){var k=c.p.grouping?c.p.groupingView:false,n,m;a.each(c.p.colModel,function(){j=this.sorttype||"text";if(j=="date"||j=="datetime"){if(this.formatter&&typeof this.formatter==="string"&&this.formatter=="date"){i=this.formatoptions&&this.formatoptions.srcformat?this.formatoptions.srcformat:
a.jgrid.formatter.date.srcformat;l=this.formatoptions&&this.formatoptions.newformat?this.formatoptions.newformat:a.jgrid.formatter.date.newformat}else i=l=this.datefmt||"Y-m-d";f[this.name]={stype:j,srcfmt:i,newfmt:l}}else f[this.name]={stype:j,srcfmt:"",newfmt:""};if(c.p.grouping){m=0;for(n=k.groupField.length;m<n;m++)if(this.name==k.groupField[m]){var b=this.name;if(typeof this.index!="undefined")b=this.index;g[m]=f[b];h[m]=b}}if(!e&&(this.index==c.p.sortname||this.name==c.p.sortname)){d=this.name;
e=true}});if(c.p.treeGrid)a(c).jqGrid("SortTree",d,c.p.sortorder,f[d].stype,f[d].srcfmt);else{var o={eq:function(a){return a.equals},ne:function(a){return a.notEquals},lt:function(a){return a.less},le:function(a){return a.lessOrEquals},gt:function(a){return a.greater},ge:function(a){return a.greaterOrEquals},cn:function(a){return a.contains},nc:function(a,c){return c==="OR"?a.orNot().contains:a.andNot().contains},bw:function(a){return a.startsWith},bn:function(a,c){return c==="OR"?a.orNot().startsWith:
a.andNot().startsWith},en:function(a,c){return c==="OR"?a.orNot().endsWith:a.andNot().endsWith},ew:function(a){return a.endsWith},ni:function(a,c){return c==="OR"?a.orNot().equals:a.andNot().equals},"in":function(a){return a.equals},nu:function(a){return a.isNull},nn:function(a,c){return c==="OR"?a.orNot().isNull:a.andNot().isNull}},p=a.jgrid.from(c.p.data);c.p.ignoreCase&&(p=p.ignoreCase());if(c.p.search===true){var q=c.p.postData.filters;if(q){typeof q=="string"&&(q=a.jgrid.parse(q));b(q)}else try{p=
o[c.p.postData.searchOper](p)(c.p.postData.searchField,c.p.postData.searchString,f[c.p.postData.searchField])}catch(s){}}if(c.p.grouping)for(m=0;m<n;m++)p.orderBy(h[m],k.groupOrder[m],g[m].stype,g[m].srcfmt);d&&c.p.sortorder&&e&&(c.p.sortorder.toUpperCase()=="DESC"?p.orderBy(c.p.sortname,"d",f[d].stype,f[d].srcfmt):p.orderBy(c.p.sortname,"a",f[d].stype,f[d].srcfmt));var q=p.select(),r=parseInt(c.p.rowNum,10),t=q.length,x=parseInt(c.p.page,10),v=Math.ceil(t/r),C={},q=q.slice((x-1)*r,x*r),f=p=null;
C[c.p.localReader.total]=v;C[c.p.localReader.page]=x;C[c.p.localReader.records]=t;C[c.p.localReader.root]=q;C[c.p.localReader.userdata]=c.p.userData;q=null;return C}}},I=function(){c.grid.hDiv.loading=true;if(!c.p.hiddengrid)switch(c.p.loadui){case "enable":a("#load_"+a.jgrid.jqID(c.p.id)).show();break;case "block":a("#lui_"+a.jgrid.jqID(c.p.id)).show();a("#load_"+a.jgrid.jqID(c.p.id)).show()}},N=function(){c.grid.hDiv.loading=false;switch(c.p.loadui){case "enable":a("#load_"+a.jgrid.jqID(c.p.id)).hide();
break;case "block":a("#lui_"+a.jgrid.jqID(c.p.id)).hide();a("#load_"+a.jgrid.jqID(c.p.id)).hide()}},P=function(b){if(!c.grid.hDiv.loading){var d=c.p.scroll&&b===false,e={},f,g=c.p.prmNames;if(c.p.page<=0)c.p.page=1;if(g.search!==null)e[g.search]=c.p.search;g.nd!==null&&(e[g.nd]=(new Date).getTime());if(g.rows!==null)e[g.rows]=c.p.rowNum;if(g.page!==null)e[g.page]=c.p.page;if(g.sort!==null)e[g.sort]=c.p.sortname;if(g.order!==null)e[g.order]=c.p.sortorder;if(c.p.rowTotal!==null&&g.totalrows!==null)e[g.totalrows]=
c.p.rowTotal;var h=a.isFunction(c.p.loadComplete),i=h?c.p.loadComplete:null,j=0,b=b||1;if(b>1)if(g.npage!==null){e[g.npage]=b;j=b-1;b=1}else i=function(a){c.p.page++;c.grid.hDiv.loading=false;h&&c.p.loadComplete.call(c,a);P(b-1)};else g.npage!==null&&delete c.p.postData[g.npage];if(c.p.grouping){a(c).jqGrid("groupingSetup");var l=c.p.groupingView,k,n="";for(k=0;k<l.groupField.length;k++){var m=l.groupField[k];a.each(c.p.colModel,function(a,c){if(c.name==m&&c.index)m=c.index});n=n+(m+" "+l.groupOrder[k]+
", ")}e[g.sort]=n+e[g.sort]}a.extend(c.p.postData,e);var o=!c.p.scroll?1:c.rows.length-1,e=a(c).triggerHandler("jqGridBeforeRequest");if(!(e===false||e==="stop"))if(a.isFunction(c.p.datatype))c.p.datatype.call(c,c.p.postData,"load_"+c.p.id);else{if(a.isFunction(c.p.beforeRequest)){e=c.p.beforeRequest.call(c);e===void 0&&(e=true);if(e===false)return}f=c.p.datatype.toLowerCase();switch(f){case "json":case "jsonp":case "xml":case "script":a.ajax(a.extend({url:c.p.url,type:c.p.mtype,dataType:f,data:a.isFunction(c.p.serializeGridData)?
c.p.serializeGridData.call(c,c.p.postData):c.p.postData,success:function(e,g,h){if(a.isFunction(c.p.beforeProcessing)&&c.p.beforeProcessing.call(c,e,g,h)===false)N();else{f==="xml"?A(e,c.grid.bDiv,o,b>1,j):F(e,c.grid.bDiv,o,b>1,j);a(c).triggerHandler("jqGridLoadComplete",[e]);i&&i.call(c,e);a(c).triggerHandler("jqGridAfterLoadComplete",[e]);d&&c.grid.populateVisible();if(c.p.loadonce||c.p.treeGrid)c.p.datatype="local";b===1&&N()}},error:function(d,e,f){a.isFunction(c.p.loadError)&&c.p.loadError.call(c,
d,e,f);b===1&&N()},beforeSend:function(b,d){var e=true;a.isFunction(c.p.loadBeforeSend)&&(e=c.p.loadBeforeSend.call(c,b,d));e===void 0&&(e=true);if(e===false)return false;I()}},a.jgrid.ajaxOptions,c.p.ajaxGridOptions));break;case "xmlstring":I();e=a.jgrid.stringToDoc(c.p.datastr);A(e,c.grid.bDiv);a(c).triggerHandler("jqGridLoadComplete",[e]);h&&c.p.loadComplete.call(c,e);a(c).triggerHandler("jqGridAfterLoadComplete",[e]);c.p.datatype="local";c.p.datastr=null;N();break;case "jsonstring":I();e=typeof c.p.datastr==
"string"?a.jgrid.parse(c.p.datastr):c.p.datastr;F(e,c.grid.bDiv);a(c).triggerHandler("jqGridLoadComplete",[e]);h&&c.p.loadComplete.call(c,e);a(c).triggerHandler("jqGridAfterLoadComplete",[e]);c.p.datatype="local";c.p.datastr=null;N();break;case "local":case "clientside":I();c.p.datatype="local";e=u();F(e,c.grid.bDiv,o,b>1,j);a(c).triggerHandler("jqGridLoadComplete",[e]);i&&i.call(c,e);a(c).triggerHandler("jqGridAfterLoadComplete",[e]);d&&c.grid.populateVisible();N()}}}},J=function(b){a("#cb_"+a.jgrid.jqID(c.p.id),
c.grid.hDiv)[c.p.useProp?"prop":"attr"]("checked",b);if(c.p.frozenColumns&&c.p.id+"_frozen")a("#cb_"+a.jgrid.jqID(c.p.id),c.grid.fhDiv)[c.p.useProp?"prop":"attr"]("checked",b)};k=function(b,d){var e="",f="<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' class='ui-pg-table'><tbody><tr>",g="",h,j,l,k,n=function(b){var d;a.isFunction(c.p.onPaging)&&(d=c.p.onPaging.call(c,b));c.p.selrow=null;if(c.p.multiselect){c.p.selarrrow=[];J(false)}c.p.savedRow=[];return d=="stop"?false:
true},b=b.substr(1),d=d+("_"+b);h="pg_"+b;j=b+"_left";l=b+"_center";k=b+"_right";a("#"+a.jgrid.jqID(b)).append("<div id='"+h+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='"+j+"' align='left'></td><td id='"+l+"' align='center' style='white-space:pre;'></td><td id='"+k+"' align='right'></td></tr></tbody></table></div>").attr("dir","ltr");if(c.p.rowList.length>
...
this.value;c.p.pager&&a(".ui-pg-selbox",c.p.pager).val(this.value);c.p.toppager&&a(".ui-pg-selbox",c.p.toppager).val(this.value);if(!n("records"))return false;P();return false});if(c.p.pgbuttons===true){a(".ui-pg-button","#"+h).hover(function(){if(a(this).hasClass("ui-state-disabled"))this.style.cursor="default";else{a(this).addClass("ui-state-hover");this.style.cursor="pointer"}},function(){if(!a(this).hasClass("ui-state-disabled")){a(this).removeClass("ui-state-hover");this.style.cursor="default"}});
a("#first"+a.jgrid.jqID(d)+", #prev"+a.jgrid.jqID(d)+", #next"+a.jgrid.jqID(d)+", #last"+a.jgrid.jqID(d)).click(function(){var a=o(c.p.page,1),b=o(c.p.lastpage,1),e=false,f=true,g=true,h=true,i=true;if(b===0||b===1)i=h=g=f=false;else if(b>1&&a>=1)if(a===1)g=f=false;else{if(a===b)i=h=false}else if(b>1&&a===0){i=h=false;a=b-1}if(this.id==="first"+d&&f){c.p.page=1;e=true}if(this.id==="prev"+d&&g){c.p.page=a-1;e=true}if(this.id==="next"+d&&h){c.p.page=a+1;e=true}if(this.id==="last"+d&&i){c.p.page=b;e=
true}if(e){if(!n(this.id))return false;P()}return false})}c.p.pginput===true&&a("input.ui-pg-input","#"+h).keypress(function(b){if((b.charCode?b.charCode:b.keyCode?b.keyCode:0)==13){c.p.page=a(this).val()>0?a(this).val():c.p.page;if(!n("user"))return false;P();return false}return this})};var X=function(b,d,e,f){if(c.p.colModel[d].sortable&&!(c.p.savedRow.length>0)){if(!e){if(c.p.lastsort==d)if(c.p.sortorder=="asc")c.p.sortorder="desc";else{if(c.p.sortorder=="desc")c.p.sortorder="asc"}else c.p.sortorder=
...
c.p.colModel[d].index||b;e=c.p.sortorder;if(a(c).triggerHandler("jqGridSortCol",[b,d,e])==="stop")c.p.lastsort=d;else if(a.isFunction(c.p.onSortCol)&&c.p.onSortCol.call(c,b,d,e)=="stop")c.p.lastsort=d;else{if(c.p.datatype=="local")c.p.deselectAfterSort&&a(c).jqGrid("resetSelection");else{c.p.selrow=null;c.p.multiselect&&J(false);c.p.selarrrow=[];c.p.savedRow=[]}if(c.p.scroll){e=c.grid.bDiv.scrollLeft;y.call(c,true,false);c.grid.hDiv.scrollLeft=e}c.p.subGrid&&c.p.datatype=="local"&&a("td.sgexpanded",
"#"+a.jgrid.jqID(c.p.id)).each(function(){a(this).trigger("click")});P();c.p.lastsort=d;if(c.p.sortname!=b&&d)c.p.lastsort=d}}},Y=function(b){var d,e={},f=a.jgrid.cellWidth()?0:c.p.cellLayout;for(d=e[0]=e[1]=e[2]=0;d<=b;d++)c.p.colModel[d].hidden===false&&(e[0]=e[0]+(c.p.colModel[d].width+f));c.p.direction=="rtl"&&(e[0]=c.p.width-e[0]);e[0]=e[0]-c.grid.bDiv.scrollLeft;a(c.grid.cDiv).is(":visible")&&(e[1]=e[1]+(a(c.grid.cDiv).height()+parseInt(a(c.grid.cDiv).css("padding-top"),10)+parseInt(a(c.grid.cDiv).css("padding-bottom"),
...
cell:"cell"}},c.p.jsonReader);c.p.localReader=a.extend(!0,{root:"rows",page:"page",total:"total",records:"records",repeatitems:!1,cell:"cell",id:"id",userdata:"userdata",subgrid:{root:"rows",repeatitems:!0,cell:"cell"}},c.p.localReader);c.p.scroll&&(c.p.pgbuttons=!1,c.p.pginput=!1,c.p.rowList=[]);c.p.data.length&&x();var L="<thead><tr class='ui-jqgrid-labels' role='rowheader'>",Z,S,C,G,M,z,w,K;S=K="";if(!0===c.p.shrinkToFit&&!0===c.p.forceFit)for(g=c.p.colModel.length-1;0<=g;g--)if(!c.p.colModel[g].hidden){c.p.colModel[g].resizable=
!1;break}"horizontal"==c.p.viewsortcols[1]&&(K=" ui-i-asc",S=" ui-i-desc");Z=m?"class='ui-th-div-ie'":"";K="<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc"+K+" ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-"+i+"'></span>"+("<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"+S+" ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-"+i+"'></span></span>");for(g=0;g<this.p.colNames.length;g++)S=c.p.headertitles?' title="'+a.jgrid.stripHtml(c.p.colNames[g])+
...
a("thead tr:first th",this).hover(function(){a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")});if(this.p.multiselect){var O=[],V;a("#cb_"+a.jgrid.jqID(c.p.id),this).bind("click",function(){
	c.p.selarrrow=[];var b=c.p.frozenColumns===true?c.p.id+"_frozen":"";
	if(this.checked){
		a(c.rows).each(function(d){
			if(d>0&&!a(this).hasClass("ui-subgrid")&&!a(this).hasClass("jqgroup")&&!a(this).hasClass("ui-state-disabled")){
				a("#jqg_"+a.jgrid.jqID(c.p.id)+"_"+a.jgrid.jqID(this.id))[c.p.useProp?"prop":"attr"]("checked",true);
		...
		a(c.rows).each(function(d){
			if(d>0&&!a(this).hasClass("ui-subgrid")&&!a(this).hasClass("ui-state-disabled")){
				a("#jqg_"+a.jgrid.jqID(c.p.id)+"_"+a.jgrid.jqID(this.id))[c.p.useProp?"prop":"attr"]("checked",false);
	...
	a(c).triggerHandler("jqGridSelectAll",[V?c.p.selarrrow:O,V]);a.isFunction(c.p.onSelectAll)&&c.p.onSelectAll.call(c,V?c.p.selarrrow:O,V)
}
)}!0===c.p.autowidth&&(L=a(n).innerWidth(),c.p.width=0<L?L:"nw");(function(){var b=0,d=a.jgrid.cellWidth()?0:o(c.p.cellLayout,0),f=0,g,i=o(c.p.scrollOffset,0),j,l=false,k,n=0,m=0,p;a.each(c.p.colModel,function(){if(typeof this.hidden==="undefined")this.hidden=false;this.widthOrg=j=o(this.width,0);if(this.hidden===false){b=b+(j+d);this.fixed?n=n+(j+d):f++;m++}});if(isNaN(c.p.width))c.p.width=b+(c.p.shrinkToFit===false&&!isNaN(c.p.height)?i:0);e.width=c.p.width;c.p.tblwidth=b;if(c.p.shrinkToFit===
false&&c.p.forceFit===true)c.p.forceFit=false;if(c.p.shrinkToFit===true&&f>0){k=e.width-d*f-n;if(!isNaN(c.p.height)){k=k-i;l=true}b=0;a.each(c.p.colModel,function(a){if(this.hidden===false&&!this.fixed){this.width=j=Math.round(k*this.width/(c.p.tblwidth-d*f-n));b=b+j;g=a}});p=0;l?e.width-n-(b+d*f)!==i&&(p=e.width-n-(b+d*f)-i):!l&&Math.abs(e.width-n-(b+d*f))!==1&&(p=e.width-n-(b+d*f));c.p.colModel[g].width=c.p.colModel[g].width+p;c.p.tblwidth=b+p+d*f+n;if(c.p.tblwidth>c.p.width){c.p.colModel[g].width=
c.p.colModel[g].width-(c.p.tblwidth-parseInt(c.p.width,10));c.p.tblwidth=c.p.width}}})();a(n).css("width",e.width+"px").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+c.p.id+"'>&#160;</div>");a(j).css("width",e.width+"px");var L=a("thead:first",c).get(0),Q="";c.p.footerrow&&(Q+="<table role='grid' style='width:"+c.p.tblwidth+"px' class='ui-jqgrid-ftable' cellspacing='0' cellpadding='0' border='0'><tbody><tr role='row' class='ui-widget-content footrow footrow-"+i+"'>");var j=a("tr:first",L),
$="<tr class='jqgfirstrow' role='row' style='height:auto'>";c.p.disableClick=!1;a("th",j).each(function(b){C=c.p.colModel[b].width;if(typeof c.p.colModel[b].resizable==="undefined")c.p.colModel[b].resizable=true;if(c.p.colModel[b].resizable){G=document.createElement("span");a(G).html("&#160;").addClass("ui-jqgrid-resize ui-jqgrid-resize-"+i);a.browser.opera||a(G).css("cursor","col-resize");a(this).addClass(c.p.resizeclass)}else G="";a(this).css("width",C+"px").prepend(G);var d="";if(c.p.colModel[b].hidden){a(this).css("display",
"none");d="display:none;"}$=$+("<td role='gridcell' style='height:0px;width:"+C+"px;"+d+"'></td>");e.headers[b]={width:C,el:this};M=c.p.colModel[b].sortable;if(typeof M!=="boolean")M=c.p.colModel[b].sortable=true;d=c.p.colModel[b].name;d=="cb"||d=="subgrid"||d=="rn"||c.p.viewsortcols[2]&&a(">div",this).addClass("ui-jqgrid-sortable");if(M)if(c.p.viewsortcols[0]){a("div span.s-ico",this).show();b==c.p.lastsort&&a("div span.ui-icon-"+c.p.sortorder,this).removeClass("ui-state-disabled")}else if(b==c.p.lastsort){a("div span.s-ico",
this).show();a("div span.ui-icon-"+c.p.sortorder,this).removeClass("ui-state-disabled")}c.p.footerrow&&(Q=Q+("<td role='gridcell' "+l(b,0,"",null,"",false)+">&#160;</td>"))}).mousedown(function(b){if(a(b.target).closest("th>span.ui-jqgrid-resize").length==1){var d=U(this);if(c.p.forceFit===true){var f=c.p,g=d,i;for(i=d+1;i<c.p.colModel.length;i++)if(c.p.colModel[i].hidden!==true){g=i;break}f.nv=g-d}e.dragStart(d,b,Y(d));return false}}).click(function(b){if(c.p.disableClick)return c.p.disableClick=
false;var d="th>div.ui-jqgrid-sortable",e,f;c.p.viewsortcols[2]||(d="th>div>span>span.ui-grid-ico-sort");b=a(b.target).closest(d);if(b.length==1){d=U(this);if(!c.p.viewsortcols[2]){e=true;f=b.attr("sort")}X(a("div",this)[0].id,d,e,f);return false}});if(c.p.sortable&&a.fn.sortable)try{a(c).jqGrid("sortableColumns",j)}catch(ba){}c.p.footerrow&&(Q+="</tr></tbody></table>");$+="</tr>";this.appendChild(document.createElement("tbody"));a(this).addClass("ui-jqgrid-btable").append($);var $=null,j=a("<table class='ui-jqgrid-htable' style='width:"+
...
c.p.pager="#"+a(c.p.pager).attr("id"),a(c.p.pager).css({width:e.width+"px"}).appendTo(n).addClass("ui-state-default ui-jqgrid-pager ui-corner-bottom"),R&&a(c.p.pager).hide(),k(c.p.pager,""));!1===c.p.cellEdit&&!0===c.p.hoverrows&&a(c).bind("mouseover",function(b){w=a(b.target).closest("tr.jqgrow");a(w).attr("class")!=="ui-subgrid"&&a(w).addClass("ui-state-hover")}).bind("mouseout",function(b){w=a(b.target).closest("tr.jqgrow");a(w).removeClass("ui-state-hover")});var H,T,ca;a(c).before(e.hDiv).click(function(b){z=
b.target;w=a(z,c.rows).closest("tr.jqgrow");if(a(w).length===0||w[0].className.indexOf("ui-state-disabled")>-1||(a(z,c).closest("table.ui-jqgrid-btable").attr("id")||"").replace("_frozen","")!==c.id)return this;var d=a(z).hasClass("cbox"),e=a(c).triggerHandler("jqGridBeforeSelectRow",[w[0].id,b]);(e=e===false||e==="stop"?false:true)&&a.isFunction(c.p.beforeSelectRow)&&(e=c.p.beforeSelectRow.call(c,w[0].id,b));if(!(z.tagName=="A"||(z.tagName=="INPUT"||z.tagName=="TEXTAREA"||z.tagName=="OPTION"||z.tagName=="SELECT")&&!d)&&e===true){H=w[0].id;T=a.jgrid.getCellIndex(z);ca=a(z).closest("td,th").html();a(c).triggerHandler("jqGridCellSelect",[H,T,ca,b]);a.isFunction(c.p.onCellSelect)&&c.p.onCellSelect.call(c,H,T,ca,b);if(c.p.cellEdit===true)if(c.p.multiselect&&d)a(c).jqGrid("setSelection",H,true,b);else{H=w[0].rowIndex;try{a(c).jqGrid("editCell",H,T,true)}catch(f){}}else if(c.p.multikey)if(b[c.p.multikey])a(c).jqGrid("setSelection",H,true,b);else{if(c.p.multiselect&&d){d=a("#jqg_"+a.jgrid.jqID(c.p.id)+"_"+H).is(":checked");a("#jqg_"+a.jgrid.jqID(c.p.id)+"_"+H)[c.p.useProp?"prop":"attr"]("checked",d)}}else{if(c.p.multiselect&&c.p.multiboxonly&&!d){var g=c.p.frozenColumns?c.p.id+"_frozen":"";a(c.p.selarrrow).each(function(b,d){var e=c.rows.namedItem(d);a(e).removeClass("ui-state-highlight");a("#jqg_"+a.jgrid.jqID(c.p.id)+"_"+a.jgrid.jqID(d))[c.p.useProp?"prop":"attr"]("checked",false);if(g){a("#"+a.jgrid.jqID(d),"#"+a.jgrid.jqID(g)).removeClass("ui-state-highlight");a("#jqg_"+a.jgrid.jqID(c.p.id)+"_"+a.jgrid.jqID(d),"#"+a.jgrid.jqID(g))[c.p.useProp?"prop":"attr"]("checked",false)}});c.p.selarrrow=[]}
			if (!this.p.multiselect) {a(c).jqGrid("setSelection",H,true,b);}
}}}).bind("reloadGrid",function(b,d){if(c.p.treeGrid===true)c.p.datatype=c.p.treedatatype;d&&d.current&&c.grid.selectionPreserver(c);if(c.p.datatype=="local"){a(c).jqGrid("resetSelection");c.p.data.length&&x()}else if(!c.p.treeGrid){c.p.selrow=null;if(c.p.multiselect){c.p.selarrrow=[];J(false)}c.p.savedRow=[]}c.p.scroll&&y.call(c,true,false);if(d&&d.page){var e=
d.page;if(e>c.p.lastpage)e=c.p.lastpage;e<1&&(e=1);c.p.page=e;c.grid.bDiv.scrollTop=c.grid.prevRowHeight?(e-1)*c.grid.prevRowHeight*c.p.rowNum:0}if(c.grid.prevRowHeight&&c.p.scroll){delete c.p.lastpage;c.grid.populateVisible()}else c.grid.populate();c.p._inlinenav===true&&a(c).jqGrid("showAddEditButtons");return false}).dblclick(function(b){z=b.target;w=a(z,c.rows).closest("tr.jqgrow");if(a(w).length!==0){H=w[0].rowIndex;T=a.jgrid.getCellIndex(z);a(c).triggerHandler("jqGridDblClickRow",[a(w).attr("id"),
H,T,b]);a.isFunction(this.p.ondblClickRow)&&c.p.ondblClickRow.call(c,a(w).attr("id"),H,T,b)}}).bind("contextmenu",function(b){z=b.target;w=a(z,c.rows).closest("tr.jqgrow");if(a(w).length!==0){c.p.multiselect||a(c).jqGrid("setSelection",w[0].id,true,b);H=w[0].rowIndex;T=a.jgrid.getCellIndex(z);a(c).triggerHandler("jqGridRightClickRow",[a(w).attr("id"),H,T,b]);a.isFunction(this.p.onRightClickRow)&&c.p.onRightClickRow.call(c,a(w).attr("id"),H,T,b)}});e.bDiv=document.createElement("div");m&&"auto"===
...
d+"px");!0===b.p.toolbar[0]&&(a(b.grid.uDiv).css("width",d+"px"),"both"==b.p.toolbar[1]&&a(b.grid.ubDiv).css("width",d+"px"));b.p.footerrow&&a(b.grid.sDiv).css("width",d+"px");!1===e&&!0===b.p.forceFit&&(b.p.forceFit=!1);if(!0===e){a.each(b.p.colModel,function(){if(this.hidden===false){f=this.widthOrg;c=c+(f+h);this.fixed?n=n+(f+h):i++;o++}});if(0===i)return;b.p.tblwidth=c;m=d-h*i-n;if(!isNaN(b.p.height)&&(a(b.grid.bDiv)[0].clientHeight<a(b.grid.bDiv)[0].scrollHeight||1===b.rows.length))j=!0,m-=k;
...
l+='"data":"'+(b+"").replace(/\\/g,"\\\\").replace(/\"/g,'\\"')+'"}';q++});l+="]}";a.extend(b.p.postData,{filters:l});a.each(["searchField","searchString","searchOper"],function(a,c){b.p.postData.hasOwnProperty(c)&&delete b.p.postData[c]})}else a.extend(b.p.postData,c);var p;b.p.searchurl&&(p=b.p.url,a(b).jqGrid("setGridParam",{url:b.p.searchurl}));var s="stop"===a(b).triggerHandler("jqGridToolbarBeforeSearch")?!0:!1;!s&&a.isFunction(d.beforeSearch)&&(s=d.beforeSearch.call(b));s||a(b).jqGrid("setGridParam",
...
i).attr(k.attr);a("select",i).css({width:"100%"});k.dataInit!==void 0&&k.dataInit(a("select",i)[0]);k.dataEvents!==void 0&&e(a("select",i)[0],k.dataEvents);d.autosearch===true&&a("select",i).change(function(){f();return false});b=null}},a.jgrid.ajaxOptions,b.p.ajaxSelectOptions||{}));else{var n,o,l;g.searchoptions?(n=void 0===g.searchoptions.value?"":g.searchoptions.value,o=void 0===g.searchoptions.separator?":":g.searchoptions.separator,l=void 0===g.searchoptions.delimiter?";":g.searchoptions.delimiter):
...
(l=document.createElement("option"),l.value=q,l.innerHTML=n[q],m.appendChild(l));void 0!==k.defaultValue&&a(m).val(k.defaultValue);k.attr&&a(m).attr(k.attr);void 0!==k.dataInit&&k.dataInit(m);void 0!==k.dataEvents&&e(m,k.dataEvents);a(i).append(m);!0===d.autosearch&&a(m).change(function(){f();return false})}}break;case "text":o=void 0!==k.defaultValue?k.defaultValue:"",a(i).append("<input type='text' style='width:95%;padding:0px;' name='"+(g.index||g.name)+"' id='gs_"+g.name+"' value='"+o+"'/>"),
...
d+"</a>":a.fn.fmatter.defaultFormat(d,e)};a.fn.fmatter.showlink=function(d,e){var b={baseLinkUrl:e.baseLinkUrl,showAction:e.showAction,addParam:e.addParam||"",target:e.target,idName:e.idName,useSlashes:e.useSlashes},f="";void 0!==e.colModel&&!a.fmatter.isUndefined(e.colModel.formatoptions)&&(b=a.extend({},b,e.colModel.formatoptions));b.target&&(f="target="+b.target);if (b.useSlashes) {b=b.baseLinkUrl+b.showAction+"/";if (b.idName!=undefined&&b.idName!=""){b+=b.idName+"/";}b+=e.rowId;if (b.addParam!=undefined&&b.addParam!="") {b+=b.addParam;}} else {b=b.baseLinkUrl+b.showAction+"?"+b.idName+"="+e.rowId+b.addParam;}return a.fmatter.isString(d)||a.fmatter.isNumber(d)?"<a "+f+' href="'+b+'">'+d+"</a>":a.fn.fmatter.defaultFormat(d,
...
(f=l.p.treeReader[o],u.hasOwnProperty(f)&&!(u[k]==i.addoper&&"parent_id_field"===o)&&delete u[f]);u[j]=a.jgrid.stripPref(l.p.idPrefix,u[j]);o=a.extend({url:d[l.p.id].url?d[l.p.id].url:a(l).jqGrid("getGridParam","editurl"),type:d[l.p.id].mtype,data:a.isFunction(d[l.p.id].serializeEditData)?d[l.p.id].serializeEditData.call(l,u):u,complete:function(f,g){u[j]=l.p.idPrefix+u[j];if(g!="success"){e[0]=false;e[1]=a(l).triggerHandler("jqGridAddEditErrorTextFormat",[f,J]);e[1]=a.isFunction(d[l.p.id].errorTextFormat)?
d[l.p.id].errorTextFormat.call(l,f):g+" Status: '"+f.statusText+"'. Error code: "+f.status}else{e=a(l).triggerHandler("jqGridAddEditAfterSubmit",[f,u,J]);e===void 0&&(e=[true,"",""]);e[0]&&a.isFunction(d[l.p.id].afterSubmit)&&(e=d[l.p.id].afterSubmit.call(l,f,u))}if(e[0]===false){a("#FormError>td",r).html(e[1]);a("#FormError",r).show()}else{a.each(l.p.colModel,function(){if(I[this.name]&&this.formatter&&this.formatter=="select")try{delete I[this.name]}catch(a){}});u=a.extend(u,I);l.p.autoencode&&
...
a(l).trigger("reloadGrid"):l.p.treeGrid===true?a(l).jqGrid("addChildNode",e[2],m,u):a(l).jqGrid("addRowData",e[2],u,b.addedrow);h("_empty",l,p)}else d[l.p.id].reloadAfterSubmit?a(l).trigger("reloadGrid"):l.p.treeGrid===true?a(l).jqGrid("addChildNode",e[2],m,u):a(l).jqGrid("addRowData",e[2],u,b.addedrow)}else{if(d[l.p.id].reloadAfterSubmit){a(l).trigger("reloadGrid");d[l.p.id].closeAfterEdit||setTimeout(function(){a(l).jqGrid("setSelection",u[j])},1E3)}else l.p.treeGrid===true?a(l).jqGrid("setTreeRow",
u[j],u):a(l).jqGrid("setRowData",u[j],u);d[l.p.id].closeAfterEdit&&a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(q),jqm:b.jqModal,onClose:d[l.p.id].onClose})}if(a.isFunction(d[l.p.id].afterComplete)){c=f;setTimeout(function(){a(l).triggerHandler("jqGridAddEditAfterComplete",[c,u,a("#"+p),J]);d[l.p.id].afterComplete.call(l,c,u,a("#"+p));c=null},500)}if(d[l.p.id].checkOnSubmit||d[l.p.id].checkOnUpdate){a("#"+p).data("disabled",false);if(d[l.p.id]._savedData[l.p.id+"_id"]!=
"_empty")for(var n in d[l.p.id]._savedData)u[n]&&(d[l.p.id]._savedData[n]=u[n])}}d[l.p.id].processing=false;a("#sData",r+"_2").removeClass("ui-state-active");try{a(":input:visible","#"+p)[0].focus()}catch(o){}}},a.jgrid.ajaxOptions,d[l.p.id].ajaxEditOptions);!o.url&&!d[l.p.id].useDataProxy&&(a.isFunction(l.p.dataProxy)?d[l.p.id].useDataProxy=!0:(e[0]=!1,e[1]+=" "+a.jgrid.errors.nourl));e[0]&&(d[l.p.id].useDataProxy?(f=l.p.dataProxy.call(l,o,"set_"+l.p.id),"undefined"==typeof f&&(f=[!0,""]),!1===f[0]?
(e[0]=!1,e[1]=f[1]||"Error deleting the selected row!"):(o.data.oper==i.addoper&&d[l.p.id].closeAfterAdd&&a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(q),jqm:b.jqModal,onClose:d[l.p.id].onClose}),o.data.oper==i.editoper&&d[l.p.id].closeAfterEdit&&a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(q),jqm:b.jqModal,onClose:d[l.p.id].onClose}))):a.ajax(o))}!1===e[0]&&(a("#FormError>td",r).html(e[1]),a("#FormError",r).show())}function j(a,b){var c=!1,
d;for(d in a)if(a[d]!=b[d]){c=!0;break}return c}function k(){var b=!0;a("#FormError",r).hide();if(d[l.p.id].checkOnUpdate&&(u={},I={},f(),N=a.extend({},u,I),P=j(N,d[l.p.id]._savedData)))a("#"+p).data("disabled",!0),a(".confirm","#"+t.themodal).show(),b=!1;return b}function m(){if("_empty"!==e&&"undefined"!==typeof l.p.savedRow&&0<l.p.savedRow.length&&a.isFunction(a.fn.jqGrid.restoreRow))for(var b=0;b<l.p.savedRow.length;b++)if(l.p.savedRow[b].id==e){a(l).jqGrid("restoreRow",e);break}}function n(b,
c){0===b?a("#pData",r+"_2").addClass("ui-state-disabled"):a("#pData",r+"_2").removeClass("ui-state-disabled");b==c?a("#nData",r+"_2").addClass("ui-state-disabled"):a("#nData",r+"_2").removeClass("ui-state-disabled")}function o(){var b=a(l).jqGrid("getDataIDs"),c=a("#id_g",r).val();return[a.inArray(c,b),b]}var l=this;if(l.grid&&e){var q=l.p.id,p="FrmGrid_"+q,s="TblGrid_"+q,r="#"+a.jgrid.jqID(s),t={themodal:"editmod"+q,modalhead:"edithd"+q,modalcontent:"editcnt"+q,scrollelm:p},v=a.isFunction(d[l.p.id].beforeShowForm)?
...
d[l.p.id].onClose);a.jgrid.viewModal("#"+a.jgrid.jqID(t.themodal),{gbox:"#gbox_"+a.jgrid.jqID(q),jqm:b.jqModal,jqM:!1,overlay:b.overlay,modal:b.modal});X||a(".jqmOverlay").click(function(){if(!k())return false;a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(q),jqm:b.jqModal,onClose:d[l.p.id].onClose});return false});a(l).triggerHandler("jqGridAddEditAfterShowForm",[a("#"+p),J]);E&&E.call(l,a("#"+p))}else{var Y=isNaN(b.dataheight)?b.dataheight:b.dataheight+"px",Y=a("<form name='FormPost' id='"+
p+"' class='FormGrid' onSubmit='return false;' style='width:100%;overflow:auto;position:relative;height:"+Y+";'></form>").data("disabled",!1),U=a("<table id='"+s+"' class='EditTable' cellspacing='0' cellpadding='0' border='0'><tbody></tbody></table>"),B=a(l).triggerHandler("jqGridAddEditBeforeInitData",[a("#"+p),J]);"undefined"==typeof B&&(B=!0);B&&y&&(B=y.call(l,a("#"+p)));if(!1===B)return;m();a(l.p.colModel).each(function(){var a=this.formoptions;A=Math.max(A,a?a.colpos||0:0);F=Math.max(F,a?a.rowpos||
0:0)});a(Y).append(U);y=a("<tr id='FormError' style='display:none'><td class='ui-state-error' colspan='"+2*A+"'></td></tr>");y[0].rp=0;a(U).append(y);y=a("<tr style='display:none' class='tinfo'><td class='topinfo' colspan='"+2*A+"'>"+d[l.p.id].topinfo+"</td></tr>");y[0].rp=0;a(U).append(y);var B=(y="rtl"==l.p.direction?!0:!1)?"nData":"pData",W=y?"pData":"nData";c(e,l,U,A);var B="<a href='javascript:void(0)' id='"+B+"' class='fm-button ui-state-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>",
...
a.jgrid.jqID(t.themodal)).hide();a("#"+p).data("disabled",false);a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(q),jqm:b.jqModal,onClose:d[l.p.id].onClose});return false});a(l).triggerHandler("jqGridAddEditInitializeForm",[a("#"+p),J]);x&&x.call(l,a("#"+p));"_empty"==e||!d[l.p.id].viewPagerButtons?a("#pData,#nData",r+"_2").hide():a("#pData,#nData",r+"_2").show();a(l).triggerHandler("jqGridAddEditBeforeShowForm",[a("#"+p),J]);v&&v.call(l,a("#"+p));a("#"+a.jgrid.jqID(t.themodal)).data("onClose",
...
function(){a(this).removeClass("ui-state-hover")});a("#sData",r+"_2").click(function(){u={};I={};a("#FormError",r).hide();f();if(u[l.p.id+"_id"]=="_empty")i();else if(b.checkOnSubmit===true){N=a.extend({},u,I);if(P=j(N,d[l.p.id]._savedData)){a("#"+p).data("disabled",true);a(".confirm","#"+a.jgrid.jqID(t.themodal)).show()}else i()}else i();return false});a("#cData",r+"_2").click(function(){if(!k())return false;a.jgrid.hideModal("#"+a.jgrid.jqID(t.themodal),{gb:"#gbox_"+a.jgrid.jqID(q),jqm:b.jqModal,
onClose:d[l.p.id].onClose});return false});a("#nData",r+"_2").click(function(){if(!k())return false;a("#FormError",r).hide();var c=o();c[0]=parseInt(c[0],10);if(c[0]!=-1&&c[1][c[0]+1]){a(l).triggerHandler("jqGridAddEditClickPgButtons",["next",a("#"+p),c[1][c[0]]]);var d=true;if(a.isFunction(b.onclickPgButtons)){d=b.onclickPgButtons.call(l,"next",a("#"+p),c[1][c[0]]);if(d!==void 0&&d===false)return false}if(a("#"+a.jgrid.jqID(c[1][c[0]+1])).hasClass("ui-state-disabled"))return false;h(c[1][c[0]+1],
l,p);a(l).jqGrid("setSelection",c[1][c[0]+1]);a(l).triggerHandler("jqGridAddEditAfterClickPgButtons",["next",a("#"+p),c[1][c[0]]]);a.isFunction(b.afterclickPgButtons)&&b.afterclickPgButtons.call(l,"next",a("#"+p),c[1][c[0]+1]);n(c[0]+1,c[1].length-1)}return false});a("#pData",r+"_2").click(function(){if(!k())return false;a("#FormError",r).hide();var c=o();if(c[0]!=-1&&c[1][c[0]-1]){a(l).triggerHandler("jqGridAddEditClickPgButtons",["prev",a("#"+p),c[1][c[0]]]);var d=true;if(a.isFunction(b.onclickPgButtons)){d=
...
b){b=a.extend({top:0,left:0,width:0,height:"auto",dataheight:"auto",modal:!1,overlay:30,drag:!0,resize:!0,jqModal:!0,closeOnEscape:!1,labelswidth:"30%",closeicon:[],navkeys:[!1,38,40],onClose:null,beforeShowForm:null,beforeInitData:null,viewPagerButtons:!0},a.jgrid.view,b||{});d[a(this)[0].p.id]=b;return this.each(function(){function f(){(!0===d[j.p.id].closeOnEscape||!0===d[j.p.id].navkeys[0])&&setTimeout(function(){a(".ui-jqdialog-titlebar-close","#"+a.jgrid.jqID(q.modalhead)).focus()},0)}function c(c,
d,e,f){for(var g,h,i,j=0,l,k,n=[],m=!1,p="<td class='CaptionTD form-view-label ui-widget-content' width='"+b.labelswidth+"'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content'>&#160;</td>",o="",s=["integer","number","currency"],r=0,t=0,q,v,y,E=1;E<=f;E++)o+=1==E?p:"<td class='CaptionTD form-view-label ui-widget-content'>&#160;</td><td class='DataTD form-view-data ui-widget-content'>&#160;</td>";a(d.p.colModel).each(function(){h=this.editrules&&!0===this.editrules.edithidden?
!1:!0===this.hidden?!0:!1;!h&&"right"===this.align&&(this.formatter&&-1!==a.inArray(this.formatter,s)?r=Math.max(r,parseInt(this.width,10)):t=Math.max(t,parseInt(this.width,10)))});q=0!==r?r:0!==t?t:0;m=a(d).jqGrid("getInd",c);a(d.p.colModel).each(function(b){g=this.name;v=!1;k=(h=this.editrules&&!0===this.editrules.edithidden?!1:!0===this.hidden?!0:!1)?"style='display:none'":"";y="boolean"!=typeof this.viewable?!0:this.viewable;if("cb"!==g&&"subgrid"!==g&&"rn"!==g&&y){l=!1===m?"":g==d.p.ExpandColumn&&
...
"trv_"+g),a(i).append(o),a(e).append(i),i[0].rp=p);a("td:eq("+(r-2)+")",i[0]).html("<b>"+("undefined"===typeof c.label?d.p.colNames[b]:c.label)+"</b>");a("td:eq("+(r-1)+")",i[0]).append("<span>"+l+"</span>").attr("id","v_"+g);v&&a("td:eq("+(r-1)+") span",i[0]).css({"text-align":"right",width:q+"px"});n[j]=b;j++}});0<j&&(c=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(2*f-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='id' value='"+
c+"'/></td></tr>"),c[0].rp=j+99,a(e).append(c));return n}function h(b,c){var d,e,f=0,g,h;if(h=a(c).jqGrid("getInd",b,!0))a("td",h).each(function(b){d=c.p.colModel[b].name;e=c.p.colModel[b].editrules&&!0===c.p.colModel[b].editrules.edithidden?!1:!0===c.p.colModel[b].hidden?!0:!1;"cb"!==d&&"subgrid"!==d&&"rn"!==d&&(g=d==c.p.ExpandColumn&&!0===c.p.treeGrid?a(this).text():a(this).html(),a.extend({},c.p.colModel[b].editoptions||{}),d=a.jgrid.jqID("v_"+d),a("#"+d+" span","#"+n).html(g),e&&a("#"+d,"#"+n).parents("tr:first").hide(),
f++)}),0<f&&a("#id_g","#"+n).val(b)}function g(b,c){0===b?a("#pData","#"+n+"_2").addClass("ui-state-disabled"):a("#pData","#"+n+"_2").removeClass("ui-state-disabled");b==c?a("#nData","#"+n+"_2").addClass("ui-state-disabled"):a("#nData","#"+n+"_2").removeClass("ui-state-disabled")}function i(){var b=a(j).jqGrid("getDataIDs"),c=a("#id_g","#"+n).val();return[a.inArray(c,b),b]}var j=this;if(j.grid&&e){var k=j.p.id,m="ViewGrid_"+a.jgrid.jqID(k),n="ViewTbl_"+a.jgrid.jqID(k),o="ViewGrid_"+k,l="ViewTbl_"+
k,q={themodal:"viewmod"+k,modalhead:"viewhd"+k,modalcontent:"viewcnt"+k,scrollelm:m},p=a.isFunction(d[j.p.id].beforeInitData)?d[j.p.id].beforeInitData:!1,s=!0,r=1,t=0;if(null!==a("#"+a.jgrid.jqID(q.themodal)).html()){p&&(s=p.call(j,a("#"+m)),"undefined"==typeof s&&(s=!0));if(!1===s)return;a(".ui-jqdialog-title","#"+a.jgrid.jqID(q.modalhead)).html(b.caption);a("#FormError","#"+n).hide();h(e,j);a.isFunction(d[j.p.id].beforeShowForm)&&d[j.p.id].beforeShowForm.call(j,a("#"+m));a.jgrid.viewModal("#"+a.jgrid.jqID(q.themodal),
{gbox:"#gbox_"+a.jgrid.jqID(k),jqm:b.jqModal,jqM:!1,overlay:b.overlay,modal:b.modal});f()}else{var v=isNaN(b.dataheight)?b.dataheight:b.dataheight+"px",o=a("<form name='FormPost' id='"+o+"' class='FormGrid' style='width:100%;overflow:auto;position:relative;height:"+v+";'></form>"),E=a("<table id='"+l+"' class='EditTable' cellspacing='1' cellpadding='2' border='0' style='table-layout:fixed'><tbody></tbody></table>");p&&(s=p.call(j,a("#"+m)),"undefined"==typeof s&&(s=!0));if(!1===s)return;a(j.p.colModel).each(function(){var a=
this.formoptions;r=Math.max(r,a?a.colpos||0:0);t=Math.max(t,a?a.rowpos||0:0)});a(o).append(E);c(e,j,E,r);l="rtl"==j.p.direction?!0:!1;p="<a href='javascript:void(0)' id='"+(l?"nData":"pData")+"' class='fm-button ui-state-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>";s="<a href='javascript:void(0)' id='"+(l?"pData":"nData")+"' class='fm-button ui-state-default ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></a>";v="<a href='javascript:void(0)' id='cData' class='fm-button ui-state-default ui-corner-all'>"+
...
a("#gview_"+a.jgrid.jqID(j.p.id))[0]);l&&(a("#pData, #nData","#"+n+"_2").css("float","right"),a(".EditButton","#"+n+"_2").css("text-align","left"));b.viewPagerButtons||a("#pData, #nData","#"+n+"_2").hide();o=null;a("#"+q.themodal).keydown(function(c){if(c.which===27){d[j.p.id].closeOnEscape&&a.jgrid.hideModal(this,{gb:b.gbox,jqm:b.jqModal,onClose:b.onClose});return false}if(b.navkeys[0]===true){if(c.which===b.navkeys[1]){a("#pData","#"+n+"_2").trigger("click");return false}if(c.which===b.navkeys[2]){a("#nData",
"#"+n+"_2").trigger("click");return false}}});b.closeicon=a.extend([!0,"left","ui-icon-close"],b.closeicon);!0===b.closeicon[0]&&a("#cData","#"+n+"_2").addClass("right"==b.closeicon[1]?"fm-button-icon-right":"fm-button-icon-left").append("<span class='ui-icon "+b.closeicon[2]+"'></span>");a.isFunction(b.beforeShowForm)&&b.beforeShowForm.call(j,a("#"+m));a.jgrid.viewModal("#"+a.jgrid.jqID(q.themodal),{gbox:"#gbox_"+a.jgrid.jqID(k),jqm:b.jqModal,modal:b.modal});a(".fm-button:not(.ui-state-disabled)",
"#"+n+"_2").hover(function(){a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")});f();a("#cData","#"+n+"_2").click(function(){a.jgrid.hideModal("#"+a.jgrid.jqID(q.themodal),{gb:"#gbox_"+a.jgrid.jqID(k),jqm:b.jqModal,onClose:b.onClose});return false});a("#nData","#"+n+"_2").click(function(){a("#FormError","#"+n).hide();var c=i();c[0]=parseInt(c[0],10);if(c[0]!=-1&&c[1][c[0]+1]){a.isFunction(b.onclickPgButtons)&&b.onclickPgButtons.call(j,"next",a("#"+m),c[1][c[0]]);
h(c[1][c[0]+1],j);a(j).jqGrid("setSelection",c[1][c[0]+1]);a.isFunction(b.afterclickPgButtons)&&b.afterclickPgButtons.call(j,"next",a("#"+m),c[1][c[0]+1]);g(c[0]+1,c[1].length-1)}f();return false});a("#pData","#"+n+"_2").click(function(){a("#FormError","#"+n).hide();var c=i();if(c[0]!=-1&&c[1][c[0]-1]){a.isFunction(b.onclickPgButtons)&&b.onclickPgButtons.call(j,"prev",a("#"+m),c[1][c[0]]);h(c[1][c[0]-1],j);a(j).jqGrid("setSelection",c[1][c[0]-1]);a.isFunction(b.afterclickPgButtons)&&b.afterclickPgButtons.call(j,
"prev",a("#"+m),c[1][c[0]-1]);g(c[0]-1,c[1].length-1)}f();return false})}o=i();g(o[0],o[1].length-1)}})},delGridRow:function(e,b){b=a.extend({top:0,left:0,width:240,height:"auto",dataheight:"auto",modal:!1,overlay:30,drag:!0,resize:!0,url:"",mtype:"POST",reloadAfterSubmit:!0,beforeShowForm:null,beforeInitData:null,afterShowForm:null,beforeSubmit:null,onclickSubmit:null,afterSubmit:null,jqModal:!0,closeOnEscape:!1,delData:{},delicon:[],cancelicon:[],onClose:null,ajaxDelOptions:{},processing:!1,serializeDelData:null,
useDataProxy:!1},a.jgrid.del,b||{});d[a(this)[0].p.id]=b;return this.each(function(){var f=this;if(f.grid&&e){var c=a.isFunction(d[f.p.id].beforeShowForm),h=a.isFunction(d[f.p.id].afterShowForm),g=a.isFunction(d[f.p.id].beforeInitData)?d[f.p.id].beforeInitData:!1,i=f.p.id,j={},k=!0,m="DelTbl_"+a.jgrid.jqID(i),n,o,l,q,p="DelTbl_"+i,s={themodal:"delmod"+i,modalhead:"delhd"+i,modalcontent:"delcnt"+i,scrollelm:m};jQuery.isArray(e)&&(e=e.join());if(null!==a("#"+a.jgrid.jqID(s.themodal)).html()){g&&(k=
g.call(f,a("#"+m)),"undefined"==typeof k&&(k=!0));if(!1===k)return;a("#DelData>td","#"+m).text(e);a("#DelError","#"+m).hide();!0===d[f.p.id].processing&&(d[f.p.id].processing=!1,a("#dData","#"+m).removeClass("ui-state-active"));c&&d[f.p.id].beforeShowForm.call(f,a("#"+m));a.jgrid.viewModal("#"+a.jgrid.jqID(s.themodal),{gbox:"#gbox_"+a.jgrid.jqID(i),jqm:d[f.p.id].jqModal,jqM:!1,overlay:d[f.p.id].overlay,modal:d[f.p.id].modal})}else{var r=isNaN(d[f.p.id].dataheight)?d[f.p.id].dataheight:d[f.p.id].dataheight+
...
"</div><span tabindex='0'><span tabindex='-1' id='jqg_alrt'></span></span>",{gbox:"#gbox_"+a.jgrid.jqID(k.p.id),jqModal:!0,drag:!0,resize:!0,caption:b.alertcap,top:b.alerttop,left:b.alertleft,width:b.alertwidth,height:b.alertheight,closeOnEscape:b.closeOnEscape,zIndex:b.alertzIndex},"","",!0));var n=1;b.cloneToTop&&k.p.toppager&&(n=2);for(var o=0;o<n;o++){var l=a("<table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table navtable' style='float:left;table-layout:auto;'><tbody><tr></tr></tbody></table>"),
q,p;0===o?(q=d,p=k.p.id,q==k.p.toppager&&(p+="_top",n=1)):(q=k.p.toppager,p=k.p.id+"_top");"rtl"==k.p.direction&&a(l).attr("dir","rtl").css("float","right");b.add&&(c=c||{},m=a("<td class='ui-pg-button ui-corner-all'></td>"),a(m).append("<div class='ui-pg-div'><span class='ui-icon "+b.addicon+"'></span>"+b.addtext+"</div>"),a("tr",l).append(m),a(m,l).attr({title:b.addtitle||"",id:c.id||"add_"+p}).click(function(){a(this).hasClass("ui-state-disabled")||(a.isFunction(b.addfunc)?b.addfunc.call(k):a(k).jqGrid("editGridRow",
"new",c));return false}).hover(function(){a(this).hasClass("ui-state-disabled")||a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")}),m=null);b.edit&&(m=a("<td class='ui-pg-button ui-corner-all'></td>"),f=f||{},a(m).append("<div class='ui-pg-div'><span class='ui-icon "+b.editicon+"'></span>"+b.edittext+"</div>"),a("tr",l).append(m),a(m,l).attr({title:b.edittitle||"",id:f.id||"edit_"+p}).click(function(){if(!a(this).hasClass("ui-state-disabled")){var c=k.p.selrow;
if(c)a.isFunction(b.editfunc)?b.editfunc.call(k,c):a(k).jqGrid("editGridRow",c,f);else{a.jgrid.viewModal("#"+j.themodal,{gbox:"#gbox_"+a.jgrid.jqID(k.p.id),jqm:true});a("#jqg_alrt").focus()}}return false}).hover(function(){a(this).hasClass("ui-state-disabled")||a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")}),m=null);b.view&&(m=a("<td class='ui-pg-button ui-corner-all'></td>"),i=i||{},a(m).append("<div class='ui-pg-div'><span class='ui-icon "+b.viewicon+"'></span>"+
...
m=null);b.del&&(m=a("<td class='ui-pg-button ui-corner-all'></td>"),h=h||{},a(m).append("<div class='ui-pg-div'><span class='ui-icon "+b.delicon+"'></span>"+b.deltext+"</div>"),a("tr",l).append(m),a(m,l).attr({title:b.deltitle||"",id:h.id||"del_"+p}).click(function(){if(!a(this).hasClass("ui-state-disabled")){var c;if(k.p.multiselect){c=k.p.selarrrow;c.length===0&&(c=null)}else c=k.p.selrow;if(c)a.isFunction(b.delfunc)?b.delfunc.call(k,c):a(k).jqGrid("delGridRow",c,h);else{a.jgrid.viewModal("#"+j.themodal,
{gbox:"#gbox_"+a.jgrid.jqID(k.p.id),jqm:true});a("#jqg_alrt").focus()}}return false}).hover(function(){a(this).hasClass("ui-state-disabled")||a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")}),m=null);(b.add||b.edit||b.del||b.view)&&a("tr",l).append("<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='ui-separator'></span></td>");b.search&&(m=a("<td class='ui-pg-button ui-corner-all'></td>"),g=g||{},a(m).append("<div class='ui-pg-div'><span class='ui-icon "+
...
a(m).append("<div class='ui-pg-div'><span class='ui-icon "+b.refreshicon+"'></span>"+b.refreshtext+"</div>"),a("tr",l).append(m),a(m,l).attr({title:b.refreshtitle||"",id:"refresh_"+p}).click(function(){if(!a(this).hasClass("ui-state-disabled")){a.isFunction(b.beforeRefresh)&&b.beforeRefresh.call(k);k.p.search=false;try{var c=k.p.id;k.p.postData.filters="";a("#fbox_"+a.jgrid.jqID(c)).jqFilter("resetFilter");a.isFunction(k.clearToolbar)&&k.clearToolbar.call(k,false)}catch(d){}switch(b.refreshstate){case "firstpage":a(k).trigger("reloadGrid",
[{page:1}]);break;case "current":a(k).trigger("reloadGrid",[{current:true}])}a.isFunction(b.afterRefresh)&&b.afterRefresh.call(k)}return false}).hover(function(){a(this).hasClass("ui-state-disabled")||a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")}),m=null);m=a(".ui-jqgrid").css("font-size")||"11px";a("body").append("<div id='testpg2' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:"+m+";visibility:hidden;' ></div>");m=a(l).clone().appendTo("#testpg2").width();
a("#testpg2").remove();a(q+"_"+b.position,q).append(l);k.p._nvtd&&(m>k.p._nvtd[0]&&(a(q+"_"+b.position,q).width(m),k.p._nvtd[0]=m),k.p._nvtd[1]=m);l=m=m=null;this.nav=!0}}}})},navButtonAdd:function(d,b){b=a.extend({caption:"newButton",title:"",buttonicon:"ui-icon-newwin",onClickButton:null,position:"last",cursor:"pointer"},b||{});return this.each(function(){if(this.grid){"string"===typeof d&&0!==d.indexOf("#")&&(d="#"+a.jgrid.jqID(d));var f=a(".navtable",d)[0],c=this;if(f&&!(b.id&&null!==a("#"+a.jgrid.jqID(b.id),
...
v=a(m).jqGrid("setRowData",d,o);a(r).attr("editable","0");for(i=0;i<m.p.savedRow.length;i++)if(m.p.savedRow[i].id==d){p=i;break}0<=p&&m.p.savedRow.splice(p,1);a(m).triggerHandler("jqGridInlineAfterSaveRow",[d,v,o,j]);a.isFunction(j.aftersavefunc)&&j.aftersavefunc.call(m,d,v);k=!0;a(r).unbind("keydown")}else a("#lui_"+a.jgrid.jqID(m.p.id)).show(),q=a.extend({},o,q),q[v]=a.jgrid.stripPref(m.p.idPrefix,q[v]),a.ajax(a.extend({url:j.url,data:a.isFunction(m.p.serializeRowData)?m.p.serializeRowData.call(m,
...
c.groupField[y];y++);l=c.groupField.length-y}for(t=0;t<l;t++)if(q[t]){v="";c.groupCollapse&&!c.showSummaryOnHide&&(v=' style="display:none;"');h+="<tr"+v+' jqfootlevel="'+(s.idx-t)+'" role="row" class="ui-widget-content jqfoot ui-row-'+f.p.direction+'">';for(var v=b(p,t,c.groups),x=f.p.colModel,B,A=v.cnt,F=0;F<e;F++){var u="<td "+f.formatCol(F,1,"")+">&#160;</td>",I="{0}";a.each(v.summary,function(){if(this.nm===x[F].name){x[F].summaryTpl&&(I=x[F].summaryTpl);"string"===typeof this.st&&"avg"===this.st.toLowerCase()&&
this.v&&0<A&&(this.v/=A);try{B=f.formatter("",this.v,F,this)}catch(b){B=this.v}u="<td "+f.formatCol(F,1,"")+">"+a.jgrid.format(I,B)+"</td>";return!1}});h+=u}h+="</tr>"}l=y}});a("#"+a.jgrid.jqID(f.p.id)+" tbody:first").append(h);h=null})},groupingGroupBy:function(d,e){return this.each(function(){"string"===typeof d&&(d=[d]);var b=this.p.groupingView;this.p.grouping=!0;"undefined"===typeof b.visibiltyOnNextGrouping&&(b.visibiltyOnNextGrouping=[]);var f;for(f=0;f<b.groupField.length;f++)!b.groupColumnShow[f]&&
